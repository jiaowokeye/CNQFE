...
class UploadS extends React.Component<Props, any> {
  ...
  state = {
    ...
    hideRenderHtml:this.props.hideRenderHtml?this.props.hideRenderHtml:false,
  ...
  getToken(){
    return Get('/psy/sys/file/upload_token')
  }
  customRequest(data){
    const _this = this;
    this.getToken().then((res)=>{
      ...
      var observer = {
        ...
        complete(res){
          ...
          _this.props.onUploadSuccess && _this.props.onUploadSuccess('http://file.emeast.com/'+res.key,label_keys,data.file.name);
        }
      }
  ...
  render () {
    const { loading,hideRenderHtml, renderHtml, renderInsideHtml, accept } = this.state;
    const prop = {};
    if (accept) {
      prop.accept = accept;
    }
    return (
      <div className='upload_box'>
        ...
        {
          !hideRenderHtml&&renderHtml && renderHtml
        }
      </div>
    );
  }
